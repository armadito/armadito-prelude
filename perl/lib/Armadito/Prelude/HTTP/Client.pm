package Armadito::Prelude::HTTP::Client;
...
sub new {
	...
	my $self = {
		logger => $params{logger} || Armadito::Prelude::Logger->new(),
		user => $params{user},
	...
	$self->{ua} = LWP::UserAgent->new(
		...
		agent                 => $Armadito::Prelude::AGENT_STRING,
...
sub _setSSLOptions {
	...
	if ( $self->{no_ssl_check} ) {
		...
		if ( $LWP::VERSION >= 6 ) {
			...
			Armadito::Prelude::HTTP::Protocol::https->use(
				ca_cert_file => $self->{ca_cert_file},
				ca_cert_dir  => $self->{ca_cert_dir},
			);

			LWP::Protocol::implementor( 'https', 'Armadito::Prelude::HTTP::Protocol::https' );
...
Armadito::Prelude::HTTP::Client - An abstract HTTP client
